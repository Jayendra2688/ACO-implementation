...
class Aco:
    def __init__(self, generations: int, ants: int, colonies: int, alpha: float, beta: float, rho: float, dist,quotas,target):
    ...
    def generate_path_for_quota(self):
        ...
        while len(to_visit) != 0 and quota<self.target:
    ...
    def run_aco(self):
        for gen in range(self.generations):
            for ant in range(self.ants):
                path_taken = self.generate_path_for_quota()
                self.update_delta(path_taken)
            print(f"\n\n")
            self.update_pheromone()

    def optimal_path(self):
        path = self.generate_path_for_quota()
