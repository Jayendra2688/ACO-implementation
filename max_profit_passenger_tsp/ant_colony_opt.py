...
class Aco:
    ...
    def generate_path(self):
        ...
        while len(to_visit) != 0:
            ...
            for next_city in next_cities:
                reward = (self.pheromones[curr_city][next_city] ** self.alpha) / (
                        (self.dist[curr_city][next_city]) ** self.beta)
                reward_sum += reward
    ...
    def calculate_return(self, path_taken):
        tot_return = 0
        for i in range(len(path_taken)):
            for j in range(i + 1, len(path_taken)):
                a = path_taken[i]
                b = path_taken[j]
                route = (a, b)
                if self.routes_info.get((a, b)) != None:
                    distance_a_b = self.dist[a][b]
                    #5Rs per Km
                    tot_passengers = self.routes_info.get((a,b))
                    return_amount = tot_passengers*(distance_a_b*5)
                    tot_return += return_amount

        return round(tot_return,2)

    def update_delta(self, path_taken):
        """adds the change in pheromone values by each ant to delta_pheromone ,so that it is useful for next
        generation"""

        len_path = self.calculate_pathlen(path_taken)
        tot_return = self.calculate_return(path_taken)

        #assuming disel cost 104/litre and mileage is 8Km/litre
        #1km expenditure appoximately 10 rs.

        tot_expend = len_path*10

        tot_profit = tot_return-tot_expend

        delta_p  = tot_profit/self.Q

        print(len_path, tot_profit)
    ...
    def optimal_path(self):
        ...
        print("Assuming 1km expenditure is 10Rs. and 1km cost of travelling is 5Rs.")
        print(f"total return : {round(self.calculate_return(path),0)}")
        print(f"total expenditure : {round(self.calculate_pathlen(path)*10,0)}")
        print(f"best Profit : {round(self.calculate_return(path),0) - round(self.calculate_pathlen(path)*10,0)}")
        return path
